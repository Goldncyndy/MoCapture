...
protocol CaptureListCollectionViewCellDelegate: AnyObject {
    func captureCollectionViewCell(_ cell: CaptureListCollectionViewCell, didTapDeleteFor event: EventsModel)
}

class CaptureListCollectionViewCell: UICollectionViewCell {
    
    static let identifier = "CaptureListCollectionViewCell"
    ...
    weak var delegate: CaptureListCollectionViewCellDelegate?
    ...
    func configure(with event: EventsModel) {
        if let imageName = event.imageName {
            ...
            if let imagePath = imagePath, let image = UIImage(contentsOfFile: imagePath) {
                ...
                imageView.image = UIImage(named: "onboard1") // Optional: Use a placeholder image
            }
        } else {
            imageView.image = UIImage(named: "onboard2") // Optional: Use a placeholder image
        }
    ...
    @objc func deleteButtonTapped() {
        ...
        delegate?.captureCollectionViewCell(self, didTapDeleteFor: event)
    }
}
